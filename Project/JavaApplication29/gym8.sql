...
--staff till now jotodin kaj korse
--staff Work Duration 
...
--ekhan theke tr jnno shob guchano ase
--query er upor je likha ta thake eta button name hbe
--for example "Show Manager" ekta button name



--ei comment er under e shob "MANAGER" TAB er under e jabe
...
--ei comment er under e shob "TRAINER" TAB er under e jabe
...
SELECT RES.TrainerId AS ID, Trainer_name, Training
FROM
...
FROM Trainer
Where Trainer_rating = (SELECT MAX(Trainer_rating) FROM Trainer)
...
SELECT TrainerId, Trainer_name, Trainer_work_hour
FROM Trainer
ORDER BY Trainer_work_hour DESC


--8 
--Trained

SELECT Trainer.TrainerID AS ID, Trainer_name, Customer_name
FROM
(SELECT TrainerId, CustomerId, Customer_name 
FROM Customers) AS RES
JOIN Trainer
ON Trainer.TrainerID = RES.TrainerID
ORDER BY ID
...
--ei comment er under e shob "EQUIPMENT" TAB er under e jabe
...
--ei comment er under e shob "STAFF" TAB er under e jabe
...
SELECT Staff_work_as, COUNT(StaffId) AS S_num
FROM Staff
GROUP BY Staff_work_as




--ei comment er under e shob "CUSTOMER" TAB er under e jabe
...
--ei button e query change hoise ektu
--eta notun kore input dis
...
SELECT Exc_type, COUNT(CustomerId) AS Exc_num
FROM Customers
GROUP BY Exc_type
ORDER BY Exc_num DESC
